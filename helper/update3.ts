...
class ProxyChecker {
  ...
  private readonly CONCURRENCY = 250;
  ...
  private readonly STATS_FILE = "./stats.json";
  ...
  private stats = { failed: 0, success: 0, total: 0 };
  ...
  private async saveResults(): Promise<void> {
    ...
    const statsData = {
      ...
      countriesCount: this.kvProxyMap.size,
    ...
    await Promise.all([
      ...
      fs.writeFile(this.STATS_FILE, JSON.stringify(statsData, null, 2)),
    ]);
  }
  ...
  public async run(): Promise<void> {
    ...
    console.log("\n✅ Proses selesai!");
    console.log(`📊 Proxy aktif: ${this.activeProxies.length}/${proxyList.length} (${((this.activeProxies.length / proxyList.length) * 100).toFixed(2)}%)`);
    console.log(`✔️  Berhasil: ${this.stats.success} | ❌ Gagal: ${this.stats.failed}`);
